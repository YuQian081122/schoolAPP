...
function getRasaServerURL() {
  ...
  // 3. 根據環境自動選擇 URL
  if (isLocalhost) {
    // 本地開發：直接連接到本地 Rasa 服務器
    const localRasaUrl = 'http://localhost:5055';
        ...
        if (parseResponse.ok) {
          ...
          if (parseData.intent && parseData.intent.name) {
            ...
            try {
              ...
              const hostname = window.location.hostname;
              const isLocalhost = hostname === 'localhost' || hostname === '127.0.0.1';
              const actionServerUrl = isLocalhost 
                ? 'http://localhost:5055/webhook'  // 本地開發
                : '/api/rasa/webhook';  // 生產環境（使用 Vercel 代理）
              
              const actionCheck = await fetch(actionServerUrl, {
                method: 'POST',
