...
function getRasaServerURL() {
  // 直接連接到 Zeabur Rasa 服務器（不通過 Vercel 代理）
  const DEFAULT_RASA_URL = 'https://rasa-service.zeabur.app';
  ...
  // 3. 默認直接連接到 Zeabur Rasa 服務器（不通過 Vercel 代理）
  Utils.logger.log(`🌐 使用默認 Rasa 伺服器（直接連接）：${DEFAULT_RASA_URL}`);
  return DEFAULT_RASA_URL;
...
async function sendToRasa(message, retryCount = 0) {
  ...
  try {
    ...
    Utils.logger.log(`📤 發送請求到 Rasa: ${rasaUrl}/webhooks/rest/webhook`);
      ...
      try {
        ...
        if (parseResponse.ok) {
          ...
          if (parseData.intent && parseData.intent.name) {
            ...
            try {
              const actionCheck = await fetch('http://localhost:5055/webhook', {
                method: 'POST',
  ...
  try {
    ...
    try {const statusResponse = await fetch(`${rasaUrl}/status`, {
      ...
      try {
        ...
        const webhookTestResponse = await fetch(`${rasaUrl}/webhooks/rest/webhook`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
...
window.diagnoseRasaConnection = async function diagnoseRasaConnection() {
  ...
  try {
    ...
    const webhookResponse = await fetch(`${rasaUrl}/webhooks/rest/webhook`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  for (const test of testMessages) {
    try {
      ...
      const testResponse = await fetch(`${rasaUrl}/webhooks/rest/webhook`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const actionServerUrl = 'http://localhost:5055';
  try {
    const actionController = new AbortController();
    const actionTimeout = setTimeout(() => actionController.abort(), 3000);
    
    const actionResponse = await fetch(`${actionServerUrl}/webhook`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  try {
    ...
    const corsResponse = await fetch(`${rasaUrl}/webhooks/rest/webhook`, {
      method: 'OPTIONS',
      headers: {
